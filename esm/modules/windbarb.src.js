...
const { defined, pushUnique, stableSort } = (external_highcharts_src_js_default_default());
/* *
 *
 *  Composition
 *
 ...
 *
 * */
class WindbarbPoint extends (external_highcharts_src_js_default_Series_types_column_default()).prototype.pointClass {
    /* *
     *
     *  Functions
     *
...
const { extend, merge, pick } = (external_highcharts_src_js_default_default());
...
function registerApproximation() {
    if (!(external_datagrouping_src_js_default_dataGrouping_approximations_default()).windbarb) {
        (external_datagrouping_src_js_default_dataGrouping_approximations_default()).windbarb = (values, directions) => {
            let vectorX = 0, vectorY = 0;
            for (let i = 0, iEnd = values.length; i < iEnd; i++) {
                vectorX += values[i] * Math.cos(directions[i] * (external_highcharts_src_js_default_default()).deg2rad);
                vectorY += values[i] * Math.sin(directions[i] * (external_highcharts_src_js_default_default()).deg2rad);
            }
            return [
                // Wind speed
                values.reduce((sum, value) => (sum + value), 0) / values.length,
                // Wind direction
                Math.atan2(vectorY, vectorX) / (external_highcharts_src_js_default_default()).deg2rad
            ];
        };
    }
}
...
external_highcharts_src_js_default_SeriesRegistry_default().registerSeriesType('windbarb', WindbarbSeries);
...
/* harmony default export */ const windbarb_src = ((external_highcharts_src_js_default_default()));

export { windbarb_src as default };
